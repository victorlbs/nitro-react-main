...
const useWiredState = () =>
{
    ...
    // Adicione ao useWiredState
    const [ areaCoords, setAreaCoords ] = useState<number[]>([]); // [x1, y1, x2, y2]

    const saveWired = () => {
    const save = (trigger: Triggerable) => {
        if(!trigger) return;

        // Se areaCoords tiver valores, use-os como parte dos intParams ou como um campo separado,
        // dependendo de como seu Emulador espera o pacote (geralmente vai nos intParams)
        const paramsToSend = areaCoords.length > 0 ? areaCoords : intParams;

        if(trigger instanceof WiredActionDefinition) {
            SendMessageComposer(new UpdateActionMessageComposer(trigger.id, paramsToSend, stringParam, furniIds, actionDelay, trigger.stuffTypeSelectionCode));
        } else if(trigger instanceof TriggerDefinition) {
            SendMessageComposer(new UpdateTriggerMessageComposer(trigger.id, paramsToSend, stringParam, furniIds, trigger.stuffTypeSelectionCode));
        } else if(trigger instanceof ConditionDefinition) {
            SendMessageComposer(new UpdateConditionMessageComposer(trigger.id, paramsToSend, stringParam, furniIds, trigger.stuffTypeSelectionCode));
        }
