...
import { useWiredArea } from '../../../hooks/wired/useWiredArea';
import { WiredSelectAreaEvent } from '@nitrots/nitro-renderer'

export interface WiredBaseViewProps {
    wiredType: string;
    requiresFurni: number;
    requiresArea?: boolean; // Adicionado
...
{
    const { wiredType = '', requiresFurni = WiredFurniType.STUFF_SELECTION_OPTION_NONE, requiresArea = false, save = null, validate = null, children = null, hasSpecialInput = false } = props;
...
// No seu WiredBaseView.tsx
const onAreaSelect = () => {
    // Dispara o evento personalizado
    Nitro.instance.events.dispatchEvent(new Event(WiredEvents.SELECT_AREA));
    
    // Opcional: Se o seu Nitro espera uma classe, você pode enviar um objeto simples:
    // Nitro.instance.events.dispatchEvent(new CustomEvent(WiredEvents.SELECT_AREA, { detail: { action: 'start' } }));
};
    ...
    return (
        <NitroCardView uniqueKey="nitro-wired" className="nitro-wired" theme="primary-slim">
            ...
            <NitroCardContentView>
                ...
                {/* Seção adicionada para Seleção de Área */}
                { requiresArea && (
                    <>
                        <hr className="m-0 bg-dark" />
                        <Column gap={ 1 }>
                            <Text bold>{ LocalizeText('Selecionar área') }</Text>
                            <Button variant="secondary" onClick={ onSelectArea }>
                                { LocalizeText('wiredfurni.pickarea.button') }
                            </Button>
                        </Column>
                    </>
                )}
