import { IMessageComposer, IRoomSession, RoomObjectVariable, RoomPreviewer, Vector3d } from '@nitrots/nitro-renderer';
import { FC, useEffect, useState } from 'react';
// ADICIONADO: GetConnection incluído nos imports
import { attemptItemPlacement, DispatchUiEvent, FurniCategory, GetConnection, GetRoomEngine, GetSessionDataManager, GroupItem, LocalizeText, SendMessageComposer, UnseenItemCategory } from '../../../../api';
...
// Nossa classe de envio oficial
class DeleteFurniComposer implements IMessageComposer<unknown[]> {
...
const attemptDeleteFurniture = (groupItem: GroupItem) =>
{
    const item = groupItem.getLastItem();
    if(!item) return;

    const confirm = window.confirm("Você tem certeza que deseja excluir este item permanentemente?");
    
    if (confirm) {
        console.log("Enviando solicitação de exclusão para o item ID:", item.id);
        SendMessageComposer(new DeleteFurniComposer(item.id));
    }
}
...
export const InventoryFurnitureView: FC<InventoryFurnitureViewProps> = props =>
{
    ...
    // INJEÇÃO DEFINITIVA DIRETAMENTE NA CONEXÃO PRINCIPAL
    useEffect(() =>
    {
        try {
            const conn = GetConnection() as any;
            
            // Injeta diretamente na gaveta de mensagens revelada pelo seu console
            if (conn && conn._messages && typeof conn._messages.registerMessageComposer === 'function') {
                conn._messages.registerMessageComposer(DeleteFurniComposer, 3000);
                console.log("✅ SUCESSO ABSOLUTO: Pacote 3000 registrado no _messages principal!");
            } 
            else if (conn && conn.messages && typeof conn.messages.registerMessageComposer === 'function') {
                conn.messages.registerMessageComposer(DeleteFurniComposer, 3000);
                console.log("✅ SUCESSO ABSOLUTO: Pacote 3000 registrado no messages principal!");
            }
            else {
                console.error("⚠️ Não foi possível achar o _messages no GetConnection()");
            }
        } catch (err) {
