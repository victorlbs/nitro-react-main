...
export const InfoStandWidgetFurniView: FC<InfoStandWidgetFurniViewProps> = props => {
    ...
    const [furniPos, setFurniPos] = useState({ x: 0, y: 0, z: 0 });
    ...
    const updatePosition = useCallback(() => {
        ...
        if (roomObject) {
            setFurniPos({
                x: roomObject.model.getValue<number>(RoomObjectVariable.LOCATION_X) || 0,
                y: roomObject.model.getValue<number>(RoomObjectVariable.LOCATION_Y) || 0,
                z: roomObject.model.getValue<number>(RoomObjectVariable.LOCATION_Z) || 0
            });
        }
    ...
    useEffect(() => {
        const onRoomObjectUpdate = (event: RoomEngineObjectEvent) => {
            if (event.objectId === avatarInfo?.id && event.category === (avatarInfo?.isWallItem ? RoomObjectCategory.WALL : RoomObjectCategory.FLOOR)) {
                updatePosition();
            }
        };

        GetRoomEngine().events.addEventListener(RoomEngineObjectEvent.OBJECT_UPDATED, onRoomObjectUpdate);
        GetRoomEngine().events.addEventListener(RoomEngineObjectEvent.OBJECT_ROLLED, onRoomObjectUpdate);

        return () => {
            GetRoomEngine().events.removeEventListener(RoomEngineObjectEvent.OBJECT_UPDATED, onRoomObjectUpdate);
            GetRoomEngine().events.removeEventListener(RoomEngineObjectEvent.OBJECT_ROLLED, onRoomObjectUpdate);
        };
    }, [avatarInfo, updatePosition]);

    useSoundEvent<NowPlayingEvent>(NowPlayingEvent.NPE_SONG_CHANGED, event => setSongId(event.id), (isJukeBox || isSongDisk));
    ...
    useEffect(() => {
        ...
        setPickupMode(pickupMode); setCanMove(canMove); setCanRotate(canRotate); setCanUse(canUse);
        setFurniKeys(furniKeyss); setFurniValues(furniValuess); setCustomKeys(customKeyss);
        setCustomValues(customValuess); setIsCrackable(isCrackable); setCrackableHits(crackableHits);
        setCrackableTarget(crackableTarget); setGodMode(godMode); setCanSeeFurniId(canSeeFurniId);
        setGroupName(null); setIsJukeBox(furniIsJukebox); setIsSongDisk(furniIsSongDisk); setSongId(furniSongId);
    ...
    const processButtonAction = useCallback((action: string) => {
        ...
        switch(action) {
            case 'buy_one': CreateLinkEvent(`catalog/open/offerId/${ avatarInfo.purchaseOfferId }`); return;
            case 'move': GetRoomEngine().processRoomObjectOperation(avatarInfo.id, avatarInfo.category, RoomObjectOperationType.OBJECT_MOVE); break;
            case 'rotate': GetRoomEngine().processRoomObjectOperation(avatarInfo.id, avatarInfo.category, RoomObjectOperationType.OBJECT_ROTATE_POSITIVE); break;
            case 'pickup': (pickupMode === PICKUP_MODE_FULL) ? GetRoomEngine().processRoomObjectOperation(avatarInfo.id, avatarInfo.category, RoomObjectOperationType.OBJECT_PICKUP) : GetRoomEngine().processRoomObjectOperation(avatarInfo.id, avatarInfo.category, RoomObjectOperationType.OBJECT_EJECT); break;
            case 'use': GetRoomEngine().useRoomObject(avatarInfo.id, avatarInfo.category); break;
            case 'save_branding_configuration': {
                const mapData = new Map<string, string>();
                const dataParts = getFurniSettingsAsString().split('\t');
                if(dataParts) { for(const part of dataParts) { const [ key, value ] = part.split('=', 2); mapData.set(key, value); } }
            ...
            case 'save_custom_variables': {
                const map = new Map();
                for(let i = 0; i < customKeys.length; i++) { if((customKeys[i] && customKeys[i].length) && (customValues[i] && customValues[i].length)) map.set(customKeys[i], customValues[i]); }
    ...
    return (
        <Column gap={ 1 } alignItems="end">
                ...
                <Column overflow="visible" className="container-fluid content-area" gap={ 1 }>
                    ...
                    <Column gap={ 1 }>
                        <Flex position="relative" gap={ 1 }>
                            { avatarInfo.stuffData.isUnique && <div className="position-absolute end-0"><LayoutLimitedEditionCompactPlateView uniqueNumber={ avatarInfo.stuffData.uniqueNumber } uniqueSeries={ avatarInfo.stuffData.uniqueSeries } /></div> }
                            { (avatarInfo.stuffData.rarityLevel > -1) && <div className="position-absolute end-0"><LayoutRarityLevelView level={ avatarInfo.stuffData.rarityLevel } /></div> }
                    ...
                    <Column gap={ 1 }>
                        <Flex alignItems="center" gap={ 1 }>
                            ...
                            <Text variant="white" small wrap>{ LocalizeText('furni.owner', [ 'name' ], [ avatarInfo.ownerName ]) }</Text>
                        </Flex>
                        { (avatarInfo.purchaseOfferId > 0) && <Flex><Text variant="white" small underline pointer onClick={ event => processButtonAction('buy_one') }>{ LocalizeText('infostand.button.buy') }</Text></Flex> }
                    </Column>
                    { (isJukeBox || isSongDisk) &&
                        <Column gap={ 1 }>
                            <hr className="m-0" />
                            { (songId === -1) && <Text variant="white" small wrap>{ LocalizeText('infostand.jukebox.text.not.playing') }</Text> }
                            { !!songName.length && <Flex alignItems="center" gap={ 1 }><Base className="icon disk-icon" /><Text variant="white" small wrap>{ songName }</Text></Flex> }
                            { !!songCreator.length && <Flex alignItems="center" gap={ 1 }><Base className="icon disk-creator" /><Text variant="white" small wrap>{ songCreator }</Text></Flex> }
                        </Column> }
                    <Column gap={ 1 }>
                        { isCrackable && <><hr className="m-0" /><Text variant="white" small wrap>{ LocalizeText('infostand.crackable_furni.hits_remaining', [ 'hits', 'target' ], [ crackableHits.toString(), crackableTarget.toString() ]) }</Text></> }
                        { avatarInfo.groupId > 0 && <><hr className="m-0" /><Flex pointer alignItems="center" gap={ 2 } onClick={ () => GetGroupInformation(avatarInfo.groupId) }><LayoutBadgeImageView badgeCode={ getGroupBadgeCode() } isGroup={ true } /><Text variant="white" underline>{ groupName }</Text></Flex></> }
                        { godMode &&
                            <>
                                <hr className="m-0" />
            ...
            <Flex gap={ 1 } justifyContent="end">
                ...
                { ((furniKeys.length > 0 && furniValues.length > 0) && (furniKeys.length === furniValues.length)) && <Button variant="dark" onClick={ () => processButtonAction('save_branding_configuration') }>{ LocalizeText('save') }</Button> }
                { ((customKeys.length > 0 && customValues.length > 0) && (customKeys.length === customValues.length)) && <Button variant="dark" onClick={ () => processButtonAction('save_custom_variables') }>{ LocalizeText('save') }</Button> }
            </Flex>
        </Column>
    );
}
